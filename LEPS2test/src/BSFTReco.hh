...
public:
  ...
  ~BSFTReco();
private:
  BSFTReco( const BSFTReco & );
  BSFTReco & operator = ( const BSFTReco & );
...
inline const TrHitContainer & BSFTReco::GetSFTTrHitContainer( int layer ) const
{
  if( layer<0 || layer>NumOfLayersSFT ) layer=-1;
  return SFTTrHitContainer_[layer];
}

inline const SFTClusterContainer & BSFTReco::GetSFTClusterContainer( int layer ) const
{
  if( layer<0 || layer>NumOfLayersSFT ) layer=-1;
  return SFTClusterContainer_[layer];
}

inline TrLocalTrack * BSFTReco::GetTrackSFTT( unsigned int i ) const
{
