...
private:
  T0Hit( const T0Hit & );
  T0Hit & operator = ( const T0Hit & );
public:
  int operator == ( const T0Hit & ) const { return 0; }
...
inline void * T0Hit::operator new( size_t )
{
  return static_cast<void *>( T0HitAllocator.MallocSingle() );
}

inline void T0Hit::operator delete( void *aHit )
{
  T0HitAllocator.
    FreeSingle( static_cast<T0Hit *>( aHit ) );
}
