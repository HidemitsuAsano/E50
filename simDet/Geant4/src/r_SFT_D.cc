/*
  ...
  10 fibers (r=0.25 mm) x 2 sublayers x 12 layers (x,u,v x,u,v, u,v,x, u,v,x)
...
r_SFT_D::r_SFT_D( const G4String & Cname,
  ...
  : Cname_(Cname)
{
  ...
  G4double Radius     = rSFT_D_Radius;

  G4double BaseThick = rSFT_BoxThick*12.0 + 20.0*mm;

  G4double radius_core_in = 0*mm;
  G4double radius_core_out = Radius*rSFT_D_Corefraction;
  G4double radius_innerclad_in = radius_core_out ;
  G4double radius_innerclad_out = radius_innerclad_in+Radius*rSFT_D_InnerCladding;
  G4double radius_outerclad_in = radius_innerclad_out ;
  G4double radius_outerclad_out =  radius_outerclad_in+Radius*rSFT_D_OuterCladding;
  G4double FrameSize = rSFT_D_FramesizeX;
  G4double FrameT = rSFT_D_FrameThick;

  G4Box *solidBox  = 
    new G4Box( Cname_+"Box", (FrameSize+1.0*mm)/2.+FrameT, (rSFT_D_Length+1.0*mm)/2.+FrameT, (BaseThick+0.5*mm)/2. );
  G4Box *solidBase  = 
    new G4Box( Cname_+"Base", FrameSize/2.+FrameT, rSFT_D_Length/2.+FrameT, BaseThick/2. );
  G4Box *solidArea  = 
    new G4Box( Cname_+"Area", FrameSize/2., rSFT_D_Length/2., BaseThick/2.+0.001*mm );
  
  
  //Note the local coordinate system is different from type A, C (square fiber) because of the symmetry.
  G4Tubs *solidLayer_core = 
    new G4Tubs( Cname_+"Layer", radius_core_in, radius_core_out, rSFT_D_Length/2.,0.*deg, 360.*deg);
  
  G4Tubs *solidLayer_innerclad = 
    new G4Tubs( Cname_+"Layer_innerclad", radius_innerclad_in, radius_innerclad_out, rSFT_D_Length/2.,0.*deg, 360.*deg);
  
  G4Tubs *solidLayer_outerclad = 
    new G4Tubs( Cname_+"Layer_outerclad", radius_outerclad_in, radius_outerclad_out, rSFT_D_Length/2.,0.*deg, 360.*deg);
  ...
  for( G4int iseg=1; iseg<=rSFT_SegNum; ++iseg ){ 
    ...
    for( G4int ilr=0; ilr<rSFT_nLayer;ilr++){
      ...
      //second sublayer
      new G4PVPlacement( layrot[conf], G4ThreeVector(  ofsScinX+offset_2ndsublayer, 0.0*mm, -LzLayer[ilr]+rSFT_D_Radius*1.732051 ),
...
void r_SFT_D::SetVisAttributes( const G4VisAttributes *attLayer,
...
void r_SFT_D::SetSensitiveDetector( G4VSensitiveDetector *SD )
{
