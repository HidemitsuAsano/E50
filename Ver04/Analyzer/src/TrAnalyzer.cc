...
bool TrAnalyzer::DecodesBRawHits( s_BeamRawHit *sbeamRaw, int type )
{
  ...
  if( type == TrackTypeBeam ){
    for( int layer=1; layer<=NumOfLayersBeamT; ++layer ){
      const s_TrRHitContainer &cont = sbeamRaw->GetsBFTRHC(layer);
      int nh=cont.size();
      for( int i=0; i<nh; ++i ){
...
bool TrAnalyzer::DecodesSRawHits( s_ScatRawHit *sscatRaw, int type )
{
  ...
  if( type == TrackTypeScat1 ){
    ...
    for( int layer=1; layer<=NumOfLayersSFT; ++layer ){
      const s_TrRHitContainer &cont=sscatRaw->GetsSFTRHC(layer);
      int nh=cont.size();
  ...
  if( type == TrackTypeScat3 ){
	...
	for( int i=0; i<nh; ++i ){
      ...
      //IT1    
      if( layer>PlMaxSFT && layer<=(PlMaxSFT + PlMaxIT1) ){
	const s_TrRHitContainer &cont=sscatRaw->GetsIT1RHC(layer-PlMaxSFT);
	int nh=cont.size();
	for( int i=0; i<nh; ++i ){
	  s_TrRawHit *rhit=cont[i];
