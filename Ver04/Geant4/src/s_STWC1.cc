...
s_STWC1::s_STWC1( const G4String & Cname,
  ...
  : Cname_(Cname)
{
  ...
  G4double LaysizeX     = ST1_LaysizeX;
  G4double LaysizeUV    = ST1_LaysizeUV;
  G4double LaysizeY     = ST1_LaysizeY;
  G4double Laythickness = ST1_LaysizeZ;

  G4double tilted    = ST1_TiltAngle;  
  G4double tilted_BT = ST1_TiltAngle_BT;  
  G4double BaseThick = ST1_BoxThick*9.0 + 20.0*mm;
  
  G4double width = LaysizeUV;//+LaysizeY*tan(tilted);
  G4double FrameSize = width;//+LaysizeY*tan(tilted);
  G4double FrameTH = ST1_FrameThickH;
  G4double FrameTV = ST1_FrameThickV;
...
void s_STWC1::SetVisAttributes( const G4VisAttributes *attLayer,
...
void s_STWC1::SetSensitiveDetector( G4VSensitiveDetector *SD )
{
