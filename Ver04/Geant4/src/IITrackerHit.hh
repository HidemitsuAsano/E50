...
#ifndef IITrackerHit_h
#define IITrackerHit_h 1
...
class IITrackerHit : public G4VHit
{
public:
  IITrackerHit();
  ~IITrackerHit() { }
private:
  IITrackerHit( const IITrackerHit & );
  IITrackerHit & operator = ( const IITrackerHit & );
public:
  int operator == ( const IITrackerHit & ) const { return 0; }
...
typedef G4THitsCollection<IITrackerHit> IITrackerHitsCollection;
extern G4Allocator<IITrackerHit> IITrackerHitAllocator;

inline void * IITrackerHit::operator new( size_t )
{
  return static_cast<void *>( IITrackerHitAllocator.MallocSingle() );
}

inline void IITrackerHit::operator delete( void *aHit )
{
  IITrackerHitAllocator.
    FreeSingle( static_cast<IITrackerHit *>( aHit ) );
}
