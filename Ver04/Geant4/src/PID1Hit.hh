...
class PID1Hit : public G4VHit
{
public:
  PID1Hit();
  ~PID1Hit() { }
private:
  PID1Hit( const PID1Hit & );
  PID1Hit & operator = ( const PID1Hit & );
public:
  int operator == ( const PID1Hit & ) const { return 0; }
...
typedef G4THitsCollection<PID1Hit> PID1HitsCollection;
extern G4Allocator<PID1Hit> PID1HitAllocator;

inline void * PID1Hit::operator new( size_t )
{
  return static_cast<void *>( PID1HitAllocator.MallocSingle() );
}

inline void PID1Hit::operator delete( void *aHit )
{
  PID1HitAllocator.
    FreeSingle( static_cast<PID1Hit *>( aHit ) );
}
