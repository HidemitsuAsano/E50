...
public:
  ...
  ~RICHHit() { }
private:
  RICHHit( const RICHHit & );
  RICHHit & operator = ( const RICHHit & );
public:
  int operator == ( const RICHHit & ) const { return 0; }
...
{
  return static_cast<void *>( RICHHitAllocator.MallocSingle() );
}

inline void RICHHit::operator delete( void *aHit )
{
  RICHHitAllocator.
    FreeSingle( static_cast<RICHHit *>( aHit ) );
}
