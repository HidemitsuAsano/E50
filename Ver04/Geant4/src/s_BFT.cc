...
s_BFT::s_BFT( const G4String & Cname,
  ...
  : Cname_(Cname)
{
  ...
  G4Box *solidBox  = 
    new G4Box( Cname_+"Box", (FrameSize+1.0*mm)/2.+FrameT, (BaseThick+1.0*mm)/2., (LaysizeY+1.0*mm)+FrameT );
  G4Box *solidBase  = 
...
void s_BFT::SetVisAttributes( const G4VisAttributes *attLayer,
...
void s_BFT::SetSensitiveDetector( G4VSensitiveDetector *SD )
{
