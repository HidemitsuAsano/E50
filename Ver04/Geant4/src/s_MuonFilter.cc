  ...
  : Cname_(Cname)
{
  ...
  G4Box *solidSegU =
    new G4Box( Cname_+"SegmentU", SegsizeX1/2., SegsizeZ1/2., SegsizeY1/2. );
  G4Box *solidSegD =
    new G4Box( Cname_+"SegmentD", SegsizeX2/2., SegsizeZ2/2., SegsizeY2/2. );
  G4Box *solidIron =
    new G4Box( Cname_+"Iron", IronsizeX/2., IronsizeZ/2., IronsizeY/2. );
  ...
  for( int id=1; id<=SegNum; ++id ){
    ...
    new G4PVPlacement( 0, G4ThreeVector( ofsScinX, ofsScinU, 0.0*mm ),
		       Cname_+"SegU", logScinU, physBox, false, 1 );
  }
  for( int id=1; id<=SegNum; ++id ){
    G4double ofsScinX=-SegSpacing*(SegNum/2.+0.5-id);
    new G4PVPlacement( 0, G4ThreeVector( ofsScinX, -ofsScinD, 0.0*mm ),
