...
void RawData::clearAll()
{
  ...
  for( int l=0; l<=PlMaxBFT; ++l){
    for_each( BFTRHC[l].begin(),  BFTRHC[l].end(), DeleteObject());
    BFTRHC[l].clear();
  }
  ...
  while( type!=EventEnd ){
	...
	if( type==FullTrackTF ){ 
	  ...
	  while( lnum!=FullTrackTFEnd ){
	    ...
	    //BFT
	    if( lnum>=PlMinBFT+PlOffsBFT && lnum<=PlMaxBFT+PlOffsBFT ){
	      double angle = geomMan.GetTiltAngle( lnum );
	      double l = x*cos(angle*Deg2Rad) + y*sin(angle*Deg2Rad);
	      dl = l + CLHEP::RandGauss::shoot( 0.0, confMan->GetBFTResol() );
	      AddTrRHit(BFTRHC[lnum-PlOffsBFT], lnum, wire, x, y, dl);
	    }
	...
	if( type==SimpleTrackSF ){
	  ...
	  while( type!=SimpleTrackSFEnd ){ 
	    ...
	    //Tracker(SFT, IT1, IT2, ST1, ST2) info
	    if( type==SimpleTrackTF ){ 
	      while( type!=SimpleTrackTFEnd ){ 
		int lnum, wire=1;
		double x, y, dl;
...
{
  if( layer<0 || layer>PlMaxBFT ) layer=0;
  return BFTRHC[layer];
}
