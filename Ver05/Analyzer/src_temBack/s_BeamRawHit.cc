...
void s_BeamRawHit::clearRegisteredHits( void )
{
  ...
  for( int l=0; l<=PlMaxBFT; ++l){
    for_each( sBFTRHC[l].begin(),  sBFTRHC[l].end(), DeleteObject());
    sBFTRHC[l].clear();
  }
}
			       ...
			       double PosX, double PosY, double DL)
{
  ...
  //BFT
  if( Layer>=PlMinBFT+PlOffsBFT && Layer<=PlMaxBFT+PlOffsBFT ){
    AddsTrRHit(sBFTRHC[Layer-PlOffsBFT], Layer, Wire, PosX, PosY, DL);
  }
...
const s_TrRHitContainer & s_BeamRawHit::GetsBFTRHC( int layer ) const
{
  if( layer<0 || layer>PlMaxBFT ) layer=0;
  return sBFTRHC[layer];
}
