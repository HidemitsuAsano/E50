...
const int Verbosity = 100;//change this value to handle console out
...
{
 if(Verbosity>0){
   std::cout << "start UserBeamTracking" << std::endl;
 }
 return true;
}
...
bool EventBeamTracking::ProcessingNormal( std::ifstream &In )
{
  ...
  if( !rawData->DecodeRawHits(In) ){
    std::cout << "FINISH Decoding raw Data " << std::endl;
    return false;
  ...
  //Beam
  {
    ...
    int nhB = cont.size();
    if(Verbosity>0){
      std::cout << __FILE__ << " line" << __LINE__ << " Beam counter # of raw hits " << nhB << std::endl;
    }
    event.gbnhits = nhB;
    for(int i=0; i<nhB; i++){
      ...
      if(Verbosity>0){
        std::cout << __FILE__ << " line" << __LINE__ << " BFT # of tracks " << nt << std::endl;
      }
      event.ntbl=nt;
