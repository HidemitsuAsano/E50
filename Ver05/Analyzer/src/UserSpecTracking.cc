...
{
  std::cout << __FILE__ << " line:" << __LINE__ << "  ProcessingNormal " << std::endl;
  const std::string funcname = "ProcessingNormal";

  rawData = new RawData;
  if( !rawData->DecodeRawHits(In) ){
    std::cout << __FILE__ << " line:" << __LINE__ << "  No raw Data" << std::endl;
    return false;
  }
  ...
  ThreeVector primPos(X0, Y0, Z0);
  {
    ...
    for(int i=0; i<nhB; i++){
      ...
      TrAna->TrackSearchBSFTT();
     
      int nt=TrAna->GetNtracksBSFTT();
      for( int it=0; it<nt; ++it ){
	TrLocalTrack *tp=TrAna->GetTrackBSFTT(it);
  ...
  // }
#if check
  std::cout<< __FILE__ << " line " << __LINE__<<  " Beam: " << BeamPCont.size() << ", pB= "<< BeamMom << std::endl;
#endif
  ...
  int ntTof=0, ntItof=0, ntPad=0, ntMF=0;
  {
    ...
    int nhS = cont.size();
    std::cout << __FILE__ << " line:" << __LINE__ << "# of Scat Raw hit " << nhS << std::endl;
    event.gsnhits = nhS;
    for(int i=0; i<nhS; i++){
      ...
      int ntSSFT     = TrAna->GetNtracksSSFTT();
      ...
      std::cout<< "ntSSFT= " << ntSSFT <<std::endl;
      ...
      int Type=0, ntScat=0;//# of track found in scatter side
      ...
      //always 0 ? (tempolary comment on Aug. 4th)
      std::cout << __FILE__ << " line: " << __LINE__ << " number of track found in scatter side " << ntScat << std::endl;
      std::cout << "hit->TrackType() " << hit->TrackType() << std::endl; 
      ...
      for( int it=0; it<ntScat; ++it ){
        ...
        if( hit->TrackType() == TrackTypeScat1 ){
          ...
          std::cout << __FILE__ << " line " << __LINE__ << "RICH hit " << nhRICH << std::endl;
          //PID1 -> AC 
          const s_HodoRHitContainer &contPID1 =hit->GetsPID1RHC();
          int nhPID1 = contPID1.size();
          if( nhPID1!=0 ) flag_pid1=true;
          std::cout << __FILE__ << " line " << __LINE__ << "PID1 hit " << nhPID1 << std::endl;
          if( flag_rich ){
#if check
          ...
          std::cout << __FILE__ << " line:" << __LINE__ << " not found Track type for PID : " << hit->TrackType()  << std::endl;
        }
        ...
        if( hit->TrackType() == TrackTypeScat2 ){
          ...
          std::cout << __FILE__ << " line:" << __LINE__ << " not found Track type  : " << hit->TrackType()  << std::endl; 
        }  
    ...
    }//for number of raw hit in scatter side
  }//Scat analysis (not a loop)
