...
bool EventSMonitor::ProcessingNormal( std::ifstream &In )
{
  ...
  //Beam
  {
    ...
    for(int i=0; i<nhB; i++){
      ...
      //BSFT
      for( int layer=1; layer<=NumOfLayersBSFT; ++layer ){
      	const s_TrRHitContainer &contBSFT = hit->GetsBSFTRHC(layer);
      	int nhBSFT=cont.size();
      	event.bftnhits = nhBSFT;
      	for( int j=0; j<nhBSFT; ++j ){
      	  s_TrRawHit *hitBSFT=contBSFT[j];
      	  event.bftlayer.push_back(hitBSFT->LayerId());

      	  int nt = hitBSFT->GetSize();
      	  for( int k=0; k<nt; k++ ) {
      	    event.bftposx.push_back(hitBSFT->GetPosX(k));
      	    event.bftposy.push_back(hitBSFT->GetPosY(k));
      	    event.bftdl.push_back(hitBSFT->GetDL(k));
      	  }
      	}
      }
    }
  ...
  //Scattered
  {
	...
	for( int k=0; k<nt; k++ ){
      ...
      for( int layer=1; layer<=NumOfLayersSSFT; ++layer ){
      	const s_TrRHitContainer &contSSFT = hit->GetsSSFTRHC(layer);
      	int nhSSFT=contSSFT.size();
      	event.sftnhits = nhSSFT;
      	for( int j=0; j<nhSSFT; ++j ){
      	  s_TrRawHit *hitSSFT=contSSFT[j];
      	  event.sftlayer.push_back(hitSSFT->LayerId());

	  //std::cout<< "sftlayer= " << hitSSFT->LayerId() << std::endl;

      	  int nt = hitSSFT->GetSize();
      	  for( int k=0; k<nt; k++ ) {
      	    event.sftposx.push_back(hitSSFT->GetPosX(k));
      	    event.sftposy.push_back(hitSSFT->GetPosY(k));
      	    event.sftdl.push_back(hitSSFT->GetDL(k));
      	  }
      	}
      }
...
{  
  ...
  //BSFT
  tree->Branch("bsftnhits", &event.bftnhits);
  tree->Branch("bsftlayer", &event.bftlayer);
  tree->Branch("bsftposx", &event.bftposx);
  tree->Branch("bsftposy", &event.bftposy);
  tree->Branch("bsftdl", &event.bftdl);

  //SSFT
  tree->Branch("ssftnhits", &event.sftnhits);
  tree->Branch("ssftlayer", &event.sftlayer);
  tree->Branch("ssftposx", &event.sftposx);
  tree->Branch("ssftposy", &event.sftposy);
  tree->Branch("ssftdl", &event.sftdl);
