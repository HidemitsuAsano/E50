...
void s_ScatRawHit::clearRegisteredHits( void )
{
  ...
  for( int l=0; l<=PlMaxSSFT; ++l){
    for_each( sSSFTRHC[l].begin(),  sSSFTRHC[l].end(), DeleteObject());
    sSSFTRHC[l].clear();
  }
			       ...
			       double PosX, double PosY, double DL)
{
  ...
  //SSFT
  if( Layer>=PlMinSSFT+PlOffsSSFT && Layer<=PlMaxSSFT+PlOffsSSFT ){
    AddsTrRHit(sSSFTRHC[Layer-PlOffsSSFT], Layer, Wire, PosX, PosY, DL);
  }
				 ...
				 int Pid, double Beta )
{
  ...
  if( DetId == geomMan.GetDetectorId("RICH") ){
    AddsHodoRHit(sRICHRHC, 
		 DetId, Layer, Seg, Time, Edep, Path, Mom, 
		 PosX, PosY, Pid, Beta);
     std::cout << __FILE__ << " line:" << __LINE__  << "found RICH hit" << std::endl;
  ...
  if( DetId == geomMan.GetDetectorId("PID1R") ||
    ...
    AddsHodoRHit(sPID1RHC, 
		 DetId, Layer, Seg, Time, Edep, Path, Mom, 
		 PosX, PosY, Pid, Beta);
     std::cout << __FILE__ << " line:" << __LINE__  << "found PID1 hit" << std::endl;
     std::cout <<"DetID " << DetId << std::endl;
     std::cout <<"Layer " << Layer << std::endl; 
     std::cout <<"Seg   " << Seg   << std::endl; 
     std::cout <<"Time  " << Time  << std::endl; 
...
const s_TrRHitContainer & s_ScatRawHit::GetsSSFTRHC( int layer ) const
{
  if( layer<0 || layer>PlMaxSSFT ) layer=0;
  return sSSFTRHC[layer];
}
