...
RICHSD::RICHSD( G4String name )
  : G4VSensitiveDetector(name)
{
  collectionName.insert( G4String( "RICHCollection" ) );
}
...
void RICHSD::Initialize( G4HCofThisEvent *HCE )
{
  static int HCID = -1;
  RICHCollection =
    new RICHHitsCollection( SensitiveDetectorName,
			    collectionName[0] );
  if( HCID<0 )
    HCID = GetCollectionID(0);
  
  HCE->AddHitsCollection( HCID, RICHCollection );
}

G4bool RICHSD::ProcessHits( G4Step *aStep,
			     G4TouchableHistory *ROhist )
{
  ...
  G4int hitLayer=0;
  if( hitName=="RICHAerogel" ) hitLayer=1;
  else if ( hitName=="RICHAerogel" ) hitLayer=1;
  G4int hitSegment = vol->GetCopyNo();

  G4int nHits = RICHCollection->entries();
...
void RICHSD::EndOfEvent( G4HCofThisEvent *HCE )
{
}
...
void RICHSD::clear()
{
  ...
  for( G4int i=nHits-1; i>=0; --i )
    delete (*RICHCollection)[i];
}

void RICHSD::DrawAll() const
{
  ...
  if( pVisManager ){
    G4int nHits = RICHCollection->entries();
    for( G4int i=0; i<nHits; ++i )
      (*RICHCollection)[i]->Draw();
  }
}

void RICHSD::PrintAll() const
{
  G4int nHits = RICHCollection->entries();
  for( G4int i=0; i<nHits; ++i)
    (*RICHCollection)[i]->Print();
}
