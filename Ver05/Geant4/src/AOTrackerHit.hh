...
#ifndef AOTrackerHit_h
#define AOTrackerHit_h 1
...
class AOTrackerHit : public G4VHit
{
public:
  AOTrackerHit();
  ~AOTrackerHit() { }
private:
  AOTrackerHit( const AOTrackerHit & );
  AOTrackerHit & operator = ( const AOTrackerHit & );
public:
  int operator == ( const AOTrackerHit & ) const { return 0; }
...
typedef G4THitsCollection<AOTrackerHit> AOTrackerHitsCollection;
extern G4Allocator<AOTrackerHit> AOTrackerHitAllocator;

inline void * AOTrackerHit::operator new( size_t )
{
  return static_cast<void *>( AOTrackerHitAllocator.MallocSingle() );
}

inline void AOTrackerHit::operator delete( void *aHit )
{
  AOTrackerHitAllocator.
    FreeSingle( static_cast<AOTrackerHit *>( aHit ) );
}
