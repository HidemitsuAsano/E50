...
SITrackerSD::SITrackerSD( G4String name )
  : G4VSensitiveDetector(name)
{
  collectionName.insert( G4String( "SITrackerCollection" ) );
}

SITrackerSD::~SITrackerSD()
{
}

void SITrackerSD::Initialize( G4HCofThisEvent *HCE )
{
  static int HCID = -1;
  SITrackerCollection =
    new SITrackerHitsCollection( SensitiveDetectorName,
			       collectionName[0] );
  if( HCID<0 )
    HCID = GetCollectionID(0);
  
  HCE->AddHitsCollection( HCID, SITrackerCollection );
}

G4bool SITrackerSD::ProcessHits( G4Step *aStep,
			     G4TouchableHistory *ROhist )
{
  ...
  G4int nHits = SITrackerCollection->entries();
  ...
  for( G4int i=0; i<nHits; ++i ){
    SITrackerHit *aHit = (*SITrackerCollection)[i];
  ...
  SITrackerHit *aHit = new SITrackerHit();
  aHit->SetPass();
  ...
  SITrackerCollection->insert( aHit );
...
void SITrackerSD::EndOfEvent( G4HCofThisEvent *HCE )
{
}

void SITrackerSD::clear()
{
  G4int nHits = SITrackerCollection->entries();
  for( G4int i=nHits-1; i>=0; --i )
    delete (*SITrackerCollection)[i];
}

void SITrackerSD::DrawAll() const
{
  ...
  if( pVisManager ){
    G4int nHits = SITrackerCollection->entries();
    for( G4int i=0; i<nHits; ++i )
      (*SITrackerCollection)[i]->Draw();
  }
}

void SITrackerSD::PrintAll() const
{
  G4int nHits = SITrackerCollection->entries();
  for( G4int i=0; i<nHits; ++i)
    (*SITrackerCollection)[i]->Print();
}
