...
s_BFTD::s_BFTD( const G4String & Cname,
  ...
  : Cname_(Cname)
{
  ...
  G4VPhysicalVolume *physLayer1 =
    new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, LzLayer1*mm ),
		       Cname_+"Layer1", logLayerU, physArea, false, id1 );
  // Layer2
  ...
  G4VPhysicalVolume *physLayer3 =
    new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, LzLayer3*mm ),
		       Cname_+"Layer3", logLayerX, physArea, false, id3 );
  // Layer4
...
void s_BFTD::SetVisAttributes( const G4VisAttributes *attLayer,
...
void s_BFTD::SetSensitiveDetector( G4VSensitiveDetector *SD )
{
