...
#ifndef SITrackerHit_h
#define SITrackerHit_h 1
...
class SITrackerHit : public G4VHit
{
public:
  SITrackerHit();
  ~SITrackerHit() { }
private:
  SITrackerHit( const SITrackerHit & );
  SITrackerHit & operator = ( const SITrackerHit & );
public:
  int operator == ( const SITrackerHit & ) const { return 0; }
...
typedef G4THitsCollection<SITrackerHit> SITrackerHitsCollection;
extern G4Allocator<SITrackerHit> SITrackerHitAllocator;

inline void * SITrackerHit::operator new( size_t )
{
  return static_cast<void *>( SITrackerHitAllocator.MallocSingle() );
}

inline void SITrackerHit::operator delete( void *aHit )
{
  SITrackerHitAllocator.
    FreeSingle( static_cast<SITrackerHit *>( aHit ) );
}
