/*
  ...
  Simple PID2 -> Just plate and box
...
s_PID2::s_PID2( const G4String & Cname, 
  ...
  : Cname_(Cname)
{
  ...
  G4ThreeVector PID2gPos=gPos+gOffset;

  //  G4cout << "OffL " << OffsetLocal << "  gOffset " << gOffset
  //	 << " gPos " << gPos << " PID2gPos " << PID2gPos
  //	 << G4endl;

  //Frame size  
  G4double BoxsizeX = PID2_BoxsizeX;
  G4double BoxsizeY = PID2_BoxsizeY;
  G4double BoxsizeZ = PID2_BoxsizeZ;
  G4double FrameT1 =  PID2_FrameThick1;

  //Radiator size
  G4double SegsizeX =  BoxsizeX-2.*FrameT1;
  G4double SegsizeY =  BoxsizeY-2.*FrameT1;
  G4double SegsizeZ =  PID2_SegsizeZ;
  ...
  //Frame box
  G4Box *solidBase1 =
    new G4Box( Cname_+"Base1", BoxsizeX/2., BoxsizeY/2., BoxsizeZ/2. );
  G4Box *solidBase2 =
    new G4Box( Cname_+"Base2", BoxsizeX/2.-FrameT1, BoxsizeY/2.-FrameT1, BoxsizeZ/2. );
  ...
  G4Box *solidSeg =
    new G4Box( Cname_+"Seg", SegsizeX/2., SegsizeY/2., SegsizeZ/2. );
  ...
  G4VPhysicalVolume *physSeg =
    new G4PVPlacement( 0, G4ThreeVector( (BoxsizeX-SegsizeX)/2.-FrameT1, 0.0*mm, (-1.)*(BoxsizeZ-SegsizeZ)/2. ),
                       Cname_+"Seg", logSeg, physArea, false, 1 );
}

void s_PID2::SetVisAttributes( const G4VisAttributes *attSeg,
...
void s_PID2::SetSensitiveDetector( G4VSensitiveDetector *SD )
{
