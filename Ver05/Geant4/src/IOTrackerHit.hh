...
#ifndef IOTrackerHit_h
#define IOTrackerHit_h 1
...
class IOTrackerHit : public G4VHit
{
public:
  IOTrackerHit();
  ~IOTrackerHit() { }
private:
  IOTrackerHit( const IOTrackerHit & );
  IOTrackerHit & operator = ( const IOTrackerHit & );
public:
  int operator == ( const IOTrackerHit & ) const { return 0; }
...
typedef G4THitsCollection<IOTrackerHit> IOTrackerHitsCollection;
extern G4Allocator<IOTrackerHit> IOTrackerHitAllocator;

inline void * IOTrackerHit::operator new( size_t )
{
  return static_cast<void *>( IOTrackerHitAllocator.MallocSingle() );
}

inline void IOTrackerHit::operator delete( void *aHit )
{
  IOTrackerHitAllocator.
    FreeSingle( static_cast<IOTrackerHit *>( aHit ) );
}
