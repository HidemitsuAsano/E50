...
class PID2Hit : public G4VHit
{
public:
  PID2Hit();
  ~PID2Hit() { }
private:
  PID2Hit( const PID2Hit & );
  PID2Hit & operator = ( const PID2Hit & );
public:
  int operator == ( const PID2Hit & ) const { return 0; }
...
typedef G4THitsCollection<PID2Hit> PID2HitsCollection;
extern G4Allocator<PID2Hit> PID2HitAllocator;

inline void * PID2Hit::operator new( size_t )
{
  return static_cast<void *>( PID2HitAllocator.MallocSingle() );
}

inline void PID2Hit::operator delete( void *aHit )
{
  PID2HitAllocator.
    FreeSingle( static_cast<PID2Hit *>( aHit ) );
}
