...
class ITofHit : public G4VHit
{
public:
  ITofHit();
  ~ITofHit() { }
private:
  ITofHit( const ITofHit & );
  ITofHit & operator = ( const ITofHit & );
public:
  int operator == ( const ITofHit & ) const { return 0; }
...
typedef G4THitsCollection<ITofHit> ITofHitsCollection;
extern G4Allocator<ITofHit> ITofHitAllocator;

inline void * ITofHit::operator new( size_t )
{
  return static_cast<void *>( ITofHitAllocator.MallocSingle() );
}

inline void ITofHit::operator delete( void *aHit )
{
  ITofHitAllocator.
    FreeSingle( static_cast<ITofHit *>( aHit ) );
}
