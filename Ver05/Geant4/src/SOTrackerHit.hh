...
#ifndef SOTrackerHit_h
#define SOTrackerHit_h 1
...
class SOTrackerHit : public G4VHit
{
public:
  SOTrackerHit();
  ~SOTrackerHit() { }
private:
  SOTrackerHit( const SOTrackerHit & );
  SOTrackerHit & operator = ( const SOTrackerHit & );
public:
  int operator == ( const SOTrackerHit & ) const { return 0; }
...
typedef G4THitsCollection<SOTrackerHit> SOTrackerHitsCollection;
extern G4Allocator<SOTrackerHit> SOTrackerHitAllocator;

inline void * SOTrackerHit::operator new( size_t )
{
  return static_cast<void *>( SOTrackerHitAllocator.MallocSingle() );
}

inline void SOTrackerHit::operator delete( void *aHit )
{
  SOTrackerHitAllocator.
    FreeSingle( static_cast<SOTrackerHit *>( aHit ) );
}
