...
private:
  PADHit( const PADHit & );
  PADHit & operator = ( const PADHit & );
public:
  int operator == ( const PADHit & ) const { return 0; }
...
typedef G4THitsCollection<PADHit> PADHitsCollection;
extern G4Allocator<PADHit> PADHitAllocator;

inline void * PADHit::operator new( size_t )
{
  return static_cast<void *>( PADHitAllocator.MallocSingle() );
}

inline void PADHit::operator delete( void *aHit )
{
  PADHitAllocator.
    FreeSingle( static_cast<PADHit *>( aHit ) );
}
